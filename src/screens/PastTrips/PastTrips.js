        ...
        try{
            ...
            .then(response => {
                ...
                if(trips.success == true){
                    //console.log(trips);
                    setpastTrips(trips.data.user_request);
                    console.log(pastTrips.length);
                }else{
                    showToast(trips.message,'error');
                }
            }).catch((err) => {
  ...
  return () => clearTimeout(timer);
    ...
    return (
        <ScrollView>
            ...
            :
                <View style={{ flex:1, justifyContent:"center", alignItems:"center"}}><Text>No Trip Found</Text></View>
                  } 
        </ScrollView>
    );
}
